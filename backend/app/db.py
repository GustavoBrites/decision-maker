...
class MockDB:
    ...
    def _seed_data(self):
        # Seed User
        user = self.create_user(CreateUserRequest(
            username="demo_user",
    ...
    def create_task(self, task_req: CreateTaskRequest, user_id: UUID) -> Task:
        task_id = uuid4()
        task = Task(
            id=task_id,
            userId=user_id,
    ...
    def get_tasks(self, user_id: UUID) -> List[Task]:
        return [t for t in self.tasks.values() if t.userId == user_id]
    ...
    def create_goal(self, goal_req: CreateGoalRequest, user_id: UUID) -> WeeklyGoal:
        goal_id = uuid4()
        goal = WeeklyGoal(
            id=goal_id,
            userId=user_id,
    ...
    def get_goals(self, user_id: UUID) -> List[WeeklyGoal]:
        return [g for g in self.goals.values() if g.userId == user_id]
